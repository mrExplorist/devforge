    ...
    <div className=" container lg:flex pt-20 w-full md:pb-10 mt-10 py-10 lg:py-16 xl:py-20 border-customGray ">
      <div className="">
        <h2 className="h2 mb-4 md:mb-8 w-3/4 ">
          ...
          <span className="gradient-text animate-gradient">irreplacable.</span>
        </h2>
        ...
        <ul className="max-w-[22rem] mb-10 md:mb-14">
          {collabContent.map((item) => (
            <li className="mb-3 py-3" key={item.id}>
              ...
              {item.text && <p className="body-2 mt-3 text-neutral-400">{item.text}</p>}
            </li>
          ))}
        </ul>
          ...
          <div className="flex w-60 aspect-square m-auto border border-neutral-600 rounded-full">
            <div className="w-[6rem] aspect-square m-auto p-[0.2rem]  bg-conic-gradient rounded-full">
              <div className="flex items-center justify-center w-full h-full bg-neutral-800 rounded-full">
                <Image src={brainwaveSymbol} width={48} height={48} alt="brainwave" />
              </div>
            </div>
          </div>
          ...
          <ul>
            {collabApps.map((app, index) => (
              ...
              >
                ...
                >
                  <Image className="m-auto" width={app.width} height={app.height} alt={app.title} src={app.icon} />
                </div>
              </li>
            ))}
          </ul>
