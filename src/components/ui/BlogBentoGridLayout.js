...
export const BentoGridItem = ({ className, title, description, header, icon, tag, date }) => {
  return (
    <div
      className={cn(
        'row-span-1 rounded-xl group/bento hover:shadow-xl transition duration-200 shadow-input dark:shadow-none p-4 border-neutral-700 border justify-between flex flex-col space-y-4 group',
        className,
      )}
    >
      {header}
      <div className="group-hover/bento:translate-x-2 transition duration-200">
        ...
        <div className="font-grotesk font-bold text-neutral-200 dark:text-neutral-200 mb-2 mt-2">{title}</div>
